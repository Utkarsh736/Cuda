...
# CUDA Kernel: Optimized Vector Addition using Element-wise Parallelism
vector_add_cuda = cpp_extension.load_inline(
    ...
    sources=[
        ...
        extern "C" __global__ void vector_add_kernel(
            ...
            if (idx < N) {
                C[idx] = __hadd(A[idx], B[idx]);  // FP16 addition using CUDA half-precision intrinsic
            }
        }
        """
    ],
...
def custom_kernel(data):
    ...
    vector_add_cuda.vector_add_kernel(
        A.contiguous(), B.contiguous(), C, N, grid=(GRID_SIZE,), block=(BLOCK_SIZE,)
    )
